...
class trainer:
    ...
    def valid_fgsm(self, loader):
        ...
        for batch_idx, (batch_imgs, batch_labels) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                predicted = self.model(batch_imgs+delta)
    ...
    def valid_pgd(self, loader):
        ...
        for batch_idx, (batch_imgs, batch_labels) in enumerate(loader):
            ...
            delta = attack_pgd(model, batch_imgs, batch_labels, data_config.epsilon, data_config.alpha, data_config.attack_iters, data_config.restarts)
            with torch.no_grad():
