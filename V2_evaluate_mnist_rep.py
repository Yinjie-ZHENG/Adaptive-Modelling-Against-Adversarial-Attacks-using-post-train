...
#
#         return sample, tgt
# data_dir = '/hdd7/yinjie/tiny-imagenet-200'
#train_dataset = TinyImageNet(data_dir, train=True)
#val_dataset = TinyImageNet(data_dir, train=False)
...
class trainer:
    ...
    def run(self, train_loder, val_loder,model_path):
        ...
        for e in range(self.config.epochs):
            e=e+start_epoch+1
            print("------model:{}----Epoch: {}--------".format(self.config.model_save_name,e))
            ...
            #err1, err5, val_loss = self.valid_fgsm(val_loder)
            #print("\nvalid_fgsm | val_loss:{:.4f} | err1:{:.4f} | err5:{:.4f}".format(val_loss, err1, err5))

            #PGD
            #if e // 20 == 0:
            #    err1, err5, val_loss = self.valid_pgd(val_loder)
            #    print("\nvalid_pgd | val_loss:{:.4f} | err1:{:.4f} | err5:{:.4f}".format(val_loss, err1, err5))
            ...
            if err1 < top_score[4][2]:
                ...
                best_err1 = save_checkpoint(self.model, self.optimizer, e, val_loss=err1, check_loss=best_err1,
                                            savepath=self.config.model_save_path, m_name=self.config.model_save_name)
...
# mnist = torchvision.datasets.MNIST(root='/hdd7/yinjie',
#                                                      train=True,
#                                                      transform=transforms.ToTensor(),
#                                                      download=True)
# mnist_loader = DataLoader(mnist, batch_size=data_config.batch_size, shuffle=True, num_workers=data_config.WORKERS, pin_memory=False)
'''
if data_config.attack == 'none':
